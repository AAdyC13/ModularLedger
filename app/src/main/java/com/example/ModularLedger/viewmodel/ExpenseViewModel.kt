// app/src/main/java/com/example/ModularLedger/viewmodel/ExpenseViewModel.kt

package com.example.ModularLedger.viewmodel
...
import com.example.ModularLedger.data.Expense
import com.example.ModularLedger.data.ExpenseRepository
 ...
 *
 * 此 ViewModel 應由 [ExpenseViewModelFactory] 建立，以注入 [ExpenseRepository] 依賴。 這是前端 (UI) 層應主要互動的 API 介面。
 *
 * @property repository [Expense] 資料的儲存庫實例。
 */
class ExpenseViewModel(private val repository: ExpenseRepository) : ViewModel() {

    /** (API-Read) 提供給 UI 層訂閱的所有 [Expense] 紀錄 (即時串流, 依時間降冪)。 */
    val allExpenses: Flow<List<Expense>> = repository.allExpenses

    /**
     * (API-Read) 依據指定的日期範圍查詢 [Expense] 紀錄。 此 API 接收 [Date] 物件，並在內部處理轉換為後端 [Repository] 所需的 Long (秒)
     * 時間戳。
     *
    ...
    /**
     * (API-Create) 建立並插入一筆新的 [Expense] 紀錄。 此函式包含業務邏輯 (資料驗證、時間戳轉換)。
     *
    ...
    fun addExpense(amount: Double, description: String, dateToSave: Date) {
        ...
        viewModelScope.launch { repository.insert(newExpense) }
    }

    /**
     * (API-Delete) 刪除一筆 [Expense] 紀錄。
 ...
 *
 * 此工廠類別是必要的，因為 [ExpenseViewModel] 具有自定義的建構子 (需要 [Application] 來取得 Repository)。 前端 (UI) 層在初始化
 * ViewModel 時必須使用此工廠。
 *
